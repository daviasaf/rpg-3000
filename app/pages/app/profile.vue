...
const { data, refresh } = await useFetch<{ profile: SocialUser; posts: ProfilePost[] }>(() => `/api/social/profile/${auth.user?.id || ''}`, { default: () => ({ profile: auth.user as SocialUser, posts: [] }) })
...
<template>
  <div class="space-y-5">
    ...
    <AppCard>
      <form class="space-y-3" @submit.prevent="publish">
        <textarea v-model="content" class="input min-h-28" placeholder="Compartilhe uma atualizacao publica do seu perfil..." />
        <AppButton type="submit"><Send class="h-4 w-4" />Publicar</AppButton>
      </form>
    </AppCard>
    ...
    <EmptyState v-if="!data.posts.length" title="Nenhum post publico" description="Publique uma atualizacao para aparecer no seu perfil social." />
  </div>
</template>
